	...
	void Update() {        
		if (Input.touchCount == 0) {
			...
			if (oldTouchPositions[0] == null || oldTouchPositions[1] != null) {
				oldTouchPositions[0] = new Vector2(GetComponent<Camera>().pixelWidth / 2, Input.GetTouch(0).position.y);
				oldTouchPositions[1] = null;
			}
			else {
				Vector2 newTouchPosition = new Vector2(GetComponent<Camera>().pixelWidth/2, Input.GetTouch(0).position.y);
                ...
                //if (GetComponent<Camera>().transform.position.y <= 0)
                //     GetComponent<Camera>().transform.Translate(new Vector3(GetComponent<Camera>().transform.position.y, 0, -10));
